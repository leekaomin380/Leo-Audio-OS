...
> **最新补充 2026-08-31：Codex已完成HIFI schema3源码、离线HAL/raw验证及8路agy探索。330主机断言+40 Python测试通过；完整SystemUI、目标签名/加载、手机验收及SRC仍未完成，不新增设备完成勾选。详见 reviews/2026-08-31-hifi-schema3-eight-way-final.md。**
...
- [ ] 在 HAL 建立可读回验证的 Leo HiFi Controller，并提供受控 HiFi 开关、HIFI 通知栏标识与只读状态显示。
  - 只读状态显示已作为可独立运行的 APK 交付（带桌面图标，34 项单测通过），
    但它不经由 HAL，走的是 USB 状态桥这一过渡实现。**HAL 侧的 Controller
    未开始**，故本条不勾。
  - **2026-08-31 用户新增需求**：HiFi 开关与通知栏 `HIFI` 字样纳入 **M3 48 kHz 基础版必交付项**，
    不等待 SRC。用户已确认是**顶部状态栏像电池旁的 HIFI 系统标识**，不是仅在下拉通知中显示文字；
    同时提供下拉快捷开关，SystemUI 最小集成与真机显示验收纳入范围。
    UI 只发送受控意图，HAL 仍是 mixer 唯一写入者；工作标识只由新鲜的实际路由证据点亮。
    **实现和上机均未完成**。上一段“HAL Controller 未开始”是历史旧状态：源码和链接候选已存在，
    尚未完成加载、控制入口和设备验收。需求、状态表与验收见
    `docs/reviews/2026-08-31-hifi-switch-notification-scope.md`。本条仍不勾，计数不变。
...
### 2026-08-31 HIFI 并发实现记录

HIFI 磁贴、顶部字形、长按 DAC 音量面板与 schema2 telemetry 已在隔离工作树实现，217 项主机检查、Java 接口编译、ARM ON/OFF/ON-repeat 通过。状态 HOST_VALIDATED / NOT_DEPLOYED；**不增加设备验收数，不将 M3/M3.5 标为完成**。仍需完整 SystemUI/ROM 构建匹配、Settings 入口、离线镜像审计和授权后的真机验收。详见本班实现验收记录。
