...
with tempfile.TemporaryDirectory(prefix='leo-hifi-tests-') as tmp:
    ...
    run([javac,'--release','8','-Xlint:-options','-d',out/'controller',*sorted((here/'controller-host').rglob('*.java')),src/'LeoHifiState.java',src/'LeoHifiRequestGate.java',src/'LeoHifiController.java',src/'LeoHifiVolumeSelection.java',here/'J152TransactionTest.java'])
    run([java,'-cp',out/'controller','com.android.systemui.leo.LeoHifiControllerHostTest'])
    run([java,'-cp',out/'controller','com.android.systemui.leo.J152TransactionTest'])
