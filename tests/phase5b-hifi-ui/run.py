...
with tempfile.TemporaryDirectory(prefix='leo-hifi-tests-') as tmp:
    ...
    run([javac,'--release','8','-Xlint:-options','-d',out/'state',src/'LeoHifiState.java',src/'LeoHifiRequestGate.java',here/'LeoHifiStateTest.java',here/'LeoHifiRequestGateTest.java'])
    for name in ['LeoHifiStateTest','LeoHifiRequestGateTest']:run([java,'-cp',out/'state','com.android.systemui.leo.'+name])
