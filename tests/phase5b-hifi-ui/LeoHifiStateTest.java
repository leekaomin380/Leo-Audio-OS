...
public class LeoHifiStateTest {
    ...
    public static void main(String[] args) {
        String validBase = "schema=2;session=123;gen=5;supported=1;requested=hifi;effective=hifi_active;live=1;flow=1;vol_ctl_l=210;vol_ctl_r=210;vol_user=40;backend=S24_LE/KHZ_48;fail=0";
        ...
        check("Missing schema", validBase.replace("schema=2;", ""), 1000, s -> !s.available);
        check("Missing session", validBase.replace("session=123;", ""), 1000, s -> !s.available);

        check("Legacy schema 1", validBase.replace("schema=2", "schema=1"), 1000, s -> !s.available);

        check("Malformed pair no equals", "schema2;session=123", 1000, s -> !s.available);
