...
public class AudioSystem {
 ...
 if(!ignore&&result==0){if(p.equals("leo_hifi_mode=false"))wire=wire.replace("requested=hifi","requested=standard").replace("effective=hifi_active","effective=wired_standard").replace("flow=1","flow=0");
 else if(p.equals("leo_hifi_mode=true"))wire=wire.replace("requested=standard","requested=hifi");}
 return result;}
}
