...
# Local sixth patch; original five-patch gate remains untouched.
if [ "$M3_PATCHES" = 1 ]; then
    [ -n "${HIFI_UI_PATCH:-}" ] || fail "HIFI_UI_PATCH required"
    git -C "$RUN/hal-tree" apply --check "$HIFI_UI_PATCH" || fail "schema2 patch mismatch"
    git -C "$RUN/hal-tree" apply "$HIFI_UI_PATCH"
    shasum -a 256 "$HIFI_UI_PATCH" > "$RUN/schema2-patch.sha256"
fi

if [ "$M3_PATCHES" = 1 ]; then
    [ -n "${HIFI_SAFETY_PATCH:-}" ] || fail "HIFI_SAFETY_PATCH required"
    git -C "$RUN/hal-tree" apply --check "$HIFI_SAFETY_PATCH" || fail "schema3 guard mismatch"
    git -C "$RUN/hal-tree" apply "$HIFI_SAFETY_PATCH"
    shasum -a 256 "$HIFI_SAFETY_PATCH" > "$RUN/schema3-patch.sha256"
fi
