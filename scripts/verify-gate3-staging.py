#!/usr/bin/env python3
"""Fail closed unless a Gate 3 staging tree equals its semantic input."""
...
def main() -> None:
    ...
    for line in args.entries.read_bytes().splitlines():
    ...
    checked_regular = 0
    checked_symlink = 0
    for_overlay = set(("app/LeoShell", "app/LeoShell/LeoShell.apk"))
    while pending:
        ...
        if info.st_mtime_ns != entry["mtime_ns"]:
            fail(f"mtime mismatch: /{relative}: {info.st_mtime_ns} != {entry['mtime_ns']}")
    ...
    actual_overlay = {path for path in observed if path in for_overlay}
    if actual_overlay != for_overlay:
        fail("the exact two Gate 3 overlay paths are not both present")
    args.report.write_text(json.dumps({
        "staging_semantic_valid": True,
        ...
        "approved_overlay_paths": sorted(actual_overlay),
